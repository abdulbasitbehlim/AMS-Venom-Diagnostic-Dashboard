...
st.title("🐍 AMS Bioelectronic scFv-Based Diagnostic Platform")
st.markdown("""
This dashboard simulates the real-time characteristic electronic signals generated by the 
specific interaction between snake venom antigens and single-chain variable fragment (scFv) antibodies using AMS Venomics kit. 
Venom concentrations are generated randomly for each run.
""")
...
# Concentration is calculated based on the amplitude to keep the relationship realistic.
...
ax.set_title('Live Sensor Feed: Antigen-scFv Binding Kinetics', fontsize=14, pad=15)
ax.set_xlabel('Time (seconds)', fontsize=12)
ax.set_ylabel('Electronic Signal (Arbitrary Units)', fontsize=12)
if selected_species:
    ax.legend(loc='upper right', title="Detected Species")
...
if selected_species:
    st.success("✅ Analysis Complete: Venom titer successfully quantified from characteristic electronic signals.")
else:
    st.warning("😀 There is no envenomation detected")

# to run this streamlit run d:/demo.py